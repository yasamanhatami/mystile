...
def blog_single(request,pid):
    ...
    post.save()
    next_post = Post.objects.filter(published_date__lte=timezone.now(),status=1)[post.id]
    previous_post = Post.objects.filter(published_date__lte=timezone.now(),status=1)[post.id-2]
